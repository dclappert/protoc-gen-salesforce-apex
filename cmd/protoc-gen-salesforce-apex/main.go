...
const MESSAGE_TEMPLATE_STRING = `// Generated by protoc-gen-salesforce-apex from {{ .Package.Name }}.{{ .Message.Name }}
...
{{- range .Message.Field }}
    global {{ propertyType $.Package $.Message . }} {{ .Name }};
{{- end }}
...
func main() {
	apiVersion = flag.String("apiVersion", "56.0", "Salesforce API version")
	flag.Parse()
...
func convertProtosToApex() {
	codeGeneratorRequest, err := toCodeGeneratorRequest(os.Stdin)
	if err != nil {
		panic(err)
	}
...
func toCodeGeneratorRequest(r io.Reader) (*pluginpb.CodeGeneratorRequest, error) {
...
func appendApexClassMetadataFile(file []*pluginpb.CodeGeneratorResponse_File, message *descriptorpb.DescriptorProto) []*pluginpb.CodeGeneratorResponse_File {
	b := bytes.NewBuffer([]byte{})
	err := classMetadataTemplate.Execute(b, classMetadataTemplateBind{ApiVersion: apiVersion})
	if err != nil {
		panic(err)
	}
	return append(file, &pluginpb.CodeGeneratorResponse_File{
